...
class Tester:
    ...
    def predict(self, test_sentence: str = None):
        ...
        input_ids = torch.tensor([custom_data.tokenized_data["input_ids"]]).to(
            self.modelLoader.device
        )
        input_mask = torch.tensor([custom_data.tokenized_data["attention_mask"]]).to(
            self.modelLoader.device
        )
